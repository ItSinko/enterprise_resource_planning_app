...
<template>
    <div class="container" @dragover.prevent="dragOver" @dragleave.prevent="dragLeave" @drop.prevent="drop($event)">
        ...
        <div v-show="Imgs.length == 0" class="beforeUpload">
            <input type="file" style="z-index: 1" accept="image/*" ref="uploadInput" @change="previewImgs" multiple />
            ...
            <p class="mainMessage">
                {{ uploadMsg ? uploadMsg : "Klik untuk Unggah Dokumen atau Seret dan Lepaskan File di Sini" }}
            </p>
        </div>
